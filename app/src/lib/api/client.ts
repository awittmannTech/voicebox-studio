...
import type {
  ActiveTasksResponse,
  AdapterInfoResponse,
  GenerationFavoriteToggle,
  GenerationNotesUpdate,
  GenerationRequest,
  GenerationResponse,
  HealthResponse,
  HistoryListResponse,
...
class ApiClient {
  ...
  async listHistory(query?: HistoryQuery): Promise<HistoryListResponse> {
    ...
    if (query?.favorites_only) params.append('favorites_only', 'true');
  ...
  async toggleFavorite(generationId: string, isFavorite: boolean): Promise<GenerationResponse> {
    return this.request<GenerationResponse>(`/history/${generationId}/favorite`, {
      method: 'PATCH',
      body: JSON.stringify({ is_favorite: isFavorite } as GenerationFavoriteToggle),
    });
  }

  async updateNotes(generationId: string, notes: string | undefined): Promise<GenerationResponse> {
    return this.request<GenerationResponse>(`/history/${generationId}/notes`, {
      method: 'PATCH',
      body: JSON.stringify({ notes } as GenerationNotesUpdate),
    });
  }
  ...
  // Training
  async startTraining(data: TrainingJobCreate): Promise<TrainingJobResponse> {
    return this.request<TrainingJobResponse>('/training/start', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
  ...
  async cancelTraining(jobId: string): Promise<{ message: string }> {
    return this.request<{ message: string }>(`/training/${jobId}/cancel`, {
      method: 'POST',
    });
  }

  async getTrainingJob(jobId: string): Promise<TrainingJobResponse> {
    return this.request<TrainingJobResponse>(`/training/${jobId}`);
  }

  async getProfileTrainingJobs(profileId: string): Promise<TrainingJobListResponse> {
    return this.request<TrainingJobListResponse>(`/profiles/${profileId}/training-jobs`);
  }

  async getProfileAdapterInfo(profileId: string): Promise<AdapterInfoResponse> {
    return this.request<AdapterInfoResponse>(`/profiles/${profileId}/adapter`);
  }

  async deleteProfileAdapter(profileId: string): Promise<{ message: string }> {
    return this.request<{ message: string }>(`/profiles/${profileId}/adapter`, {
      method: 'DELETE',
    });
  }
}
