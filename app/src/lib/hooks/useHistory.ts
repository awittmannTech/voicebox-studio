...
export function useToggleFavorite() {
  ...
  return useMutation({
    mutationFn: ({ generationId, isFavorite }: { generationId: string; isFavorite: boolean }) =>
      apiClient.toggleFavorite(generationId, isFavorite),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['history'] });
    },
  });
...
export function useUpdateNotes() {
  ...
  return useMutation({
    mutationFn: ({ generationId, notes }: { generationId: string; notes: string | undefined }) =>
      apiClient.updateNotes(generationId, notes),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['history'] });
    },
  });
