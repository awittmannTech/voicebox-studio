...
export function HistoryTable() {
  ...
  const [editingNotesGen, setEditingNotesGen] = useState<{ id: string; notes: string } | null>(null);
  const limit = 20;
  ...
  // Reset to page 0 when deletions, imports, or filter changes occur
  useEffect(() => {
  ...
  const handleToggleFavorite = (e: React.MouseEvent, gen: HistoryResponse) => {
    e.stopPropagation();
    // Optimistically update local state
    setAllHistory((prev) =>
      prev.map((item) =>
        item.id === gen.id ? { ...item, is_favorite: !item.is_favorite } : item,
      ),
    );
    toggleFavorite.mutate({ generationId: gen.id, isFavorite: !gen.is_favorite });
  };

  const handleOpenNotes = (gen: HistoryResponse) => {
    setEditingNotesGen({ id: gen.id, notes: gen.notes || '' });
  ...
  const handleSaveNotes = () => {
    if (editingNotesGen) {
      updateNotes.mutate({
        generationId: editingNotesGen.id,
        notes: editingNotesGen.notes || undefined,
      });
      setNotesDialogOpen(false);
      setEditingNotesGen(null);
    }
  };
    ...
    <div className="flex flex-col h-full min-h-0 relative">
      ...
      {history.length === 0 ? (
        <div className="text-center py-12 px-5 border-2 border-dashed mb-5 border-muted rounded-md text-muted-foreground flex-1 flex items-center justify-center">
          {favoritesOnly ? 'No favorite generations yet' : 'No voice generations, yet...'}
        </div>
      ) : (
        <>
          {isScrolled && (
              ...
              return (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={(e) => handleToggleFavorite(e, gen)}
                  ...
                  >
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        >
                          <Star className={cn('mr-2 h-4 w-4', gen.is_favorite && 'fill-yellow-400 text-yellow-400')} />
                          {gen.is_favorite ? 'Unfavorite' : 'Favorite'}
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleOpenNotes(gen)}>
                          <StickyNote className="mr-2 h-4 w-4" />
                          {gen.notes ? 'Edit Notes' : 'Add Notes'}
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem
      ...
      <Dialog open={notesDialogOpen} onOpenChange={setNotesDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Generation Notes</DialogTitle>
            <DialogDescription>
              Add notes to this generation for future reference.
            </DialogDescription>
          </DialogHeader>
          <Textarea
            value={editingNotesGen?.notes || ''}
            onChange={(e) =>
              setEditingNotesGen((prev) =>
