...
export function ModelManagement() {
  ...
  // Safety check: Clear local downloading state if backend confirms download is complete
  // This prevents stuck "Downloading..." UI when SSE callbacks fail or server restarts
  useEffect(() => {
    if (downloadingModel && modelStatus) {
      const model = modelStatus.models.find(m => m.model_name === downloadingModel);
      // If backend says downloaded and not downloading, clear our local state
      if (model && model.downloaded && !model.downloading) {
        console.log('[ModelManagement] Backend confirms download complete, clearing stale local state');
        setDownloadingModel(null);
        setDownloadingDisplayName(null);
      }
    }
  }, [modelStatus, downloadingModel]);
