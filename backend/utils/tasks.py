...
@dataclass
class TrainingTask:
    """Represents an active training task."""
    job_id: str
    profile_id: str
    status: str = "training"  # training, complete, error
    started_at: datetime = field(default_factory=datetime.utcnow)
    error: Optional[str] = None
...
class TaskManager:
    ...
    def __init__(self):
        ...
        self._active_trainings: Dict[str, TrainingTask] = {}
    ...
    def clear_error_downloads(self) -> int:
        """Clear all downloads with error status. Returns count of cleared tasks."""
        error_models = [
            model_name for model_name, task in self._active_downloads.items()
    ...
    def clear_all_downloads(self) -> int:
        """Clear all download tasks regardless of status. Returns count of cleared tasks."""
        count = len(self._active_downloads)
        self._active_downloads.clear()
        return count

    # Training task management
    def start_training(self, job_id: str, profile_id: str) -> None:
        """Mark a training job as started."""
        self._active_trainings[job_id] = TrainingTask(
            job_id=job_id,
            profile_id=profile_id,
            status="training",
        )

    def complete_training(self, job_id: str) -> None:
        """Mark a training job as complete."""
        if job_id in self._active_trainings:
            del self._active_trainings[job_id]

    def error_training(self, job_id: str, error: str) -> None:
        """Mark a training job as failed."""
        if job_id in self._active_trainings:
            self._active_trainings[job_id].status = "error"
            self._active_trainings[job_id].error = error

    def get_active_trainings(self) -> List[TrainingTask]:
        """Get all active training jobs."""
        return list(self._active_trainings.values())

    def is_training_active(self) -> bool:
        """Check if any training job is currently active."""
        return len(self._active_trainings) > 0
