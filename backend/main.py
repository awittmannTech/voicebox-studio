...
from . import database, models, profiles, history, tts, transcribe, config, export_import, channels, stories, hotkey_recordings, training, __version__
...
async def generate_speech(
    ...
    progress_key = f"generation:{generation_id}"

    def update_gen_progress(stage: str, progress: float = 0):
        """Helper to update generation progress via ProgressManager."""
    ...
    try:
        # Check if training is active (blocks generation)
        task_manager = get_task_manager()
        if task_manager.is_training_active():
            raise HTTPException(
                status_code=409,
                detail="Cannot generate while training is in progress. Please wait for training to complete."
            )
        ...
        update_gen_progress("loading_model", 0)
        ...
        update_gen_progress("creating_voice_prompt", 20)
        ...
        update_gen_progress("generating_audio", 40)

        # Load adapter if profile has one and use_adapter is enabled
        adapter_loaded = False
        if data.use_adapter is not False and profile.adapter_path and Path(profile.adapter_path).exists():
            try:
                training.load_adapter_for_inference(data.profile_id)
                adapter_loaded = True
            except Exception as e:
                # Non-fatal: continue without adapter
                print(f"Warning: Failed to load adapter for profile {data.profile_id}: {e}")
        ...
        try:
            ...
            # Unload adapter after generation to restore base model
            if adapter_loaded:
                try:
                    training.unload_adapter(data.profile_id)
                except Exception as e:
                    print(f"Warning: Failed to unload adapter: {e}")

        update_gen_progress("saving", 80)
        ...
        update_gen_progress("complete", 100)
...
async def get_generation_progress(generation_id: str):
    """Get generation progress via Server-Sent Events."""
    progress_manager = get_progress_manager()
    progress_key = f"generation:{generation_id}"

    async def event_generator():
        """Generate SSE events for generation progress updates."""
        async for event in progress_manager.subscribe(progress_key):
            yield event
...
@app.patch("/history/{generation_id}/favorite", response_model=models.GenerationResponse)
async def toggle_generation_favorite(
    generation_id: str,
    data: models.GenerationFavoriteToggle,
    db: Session = Depends(get_db),
):
    """Toggle favorite status for a generation."""
    result = await history.toggle_favorite(generation_id, data.is_favorite, db)
    if not result:
        raise HTTPException(status_code=404, detail="Generation not found")
    return result


@app.patch("/history/{generation_id}/notes", response_model=models.GenerationResponse)
async def update_generation_notes(
    generation_id: str,
    data: models.GenerationNotesUpdate,
    db: Session = Depends(get_db),
):
    """Update notes for a generation."""
    result = await history.update_notes(generation_id, data.notes, db)
    if not result:
        raise HTTPException(status_code=404, detail="Generation not found")
    return result
...
@app.post("/hotkey-recordings", response_model=models.HotkeyRecordingResponse)
async def create_hotkey_recording(
    audio: UploadFile = File(...),
    duration: float = Form(...),
    db: Session = Depends(get_db),
):
    """Upload and save a hotkey recording."""
    try:
        # Read audio data
        audio_data = await audio.read()

        # Create recording
        recording = hotkey_recordings.create_recording(db, audio_data, duration)
...
@app.get("/hotkey-recordings", response_model=models.HotkeyRecordingListResponse)
async def list_hotkey_recordings(
    skip: int = 0,
    limit: int = 50,
    db: Session = Depends(get_db),
):
    """List all hotkey recordings (paginated, newest first)."""
    recordings, total = hotkey_recordings.list_recordings(db, skip, limit)
    return models.HotkeyRecordingListResponse(items=recordings, total=total)


@app.get("/hotkey-recordings/{recording_id}", response_model=models.HotkeyRecordingResponse)
async def get_hotkey_recording(
    recording_id: str,
    db: Session = Depends(get_db),
):
    """Get a single hotkey recording."""
    recording = hotkey_recordings.get_recording(db, recording_id)
    if not recording:
        raise HTTPException(status_code=404, detail="Recording not found")
    return recording


@app.get("/hotkey-recordings/{recording_id}/audio")
async def get_hotkey_recording_audio(
    recording_id: str,
    db: Session = Depends(get_db),
):
    """Download hotkey recording audio file."""
    recording = hotkey_recordings.get_recording(db, recording_id)
    if not recording:
        raise HTTPException(status_code=404, detail="Recording not found")
    ...
    return FileResponse(
        audio_path,
        media_type="audio/wav",
        filename=f"hotkey_recording_{recording_id}.wav",
    )


@app.post("/hotkey-recordings/{recording_id}/transcribe", response_model=models.HotkeyRecordingResponse)
async def transcribe_hotkey_recording(
    recording_id: str,
    data: models.HotkeyRecordingTranscribe,
    db: Session = Depends(get_db),
):
    """Transcribe a hotkey recording."""
    recording = hotkey_recordings.get_recording(db, recording_id)
    if not recording:
        raise HTTPException(status_code=404, detail="Recording not found")
    ...
    try:
        # Transcribe audio
        transcription_result = await transcribe.transcribe_audio(
            audio_path,
            language=data.language
        )

        # Update recording with transcription
        updated_recording = hotkey_recordings.update_transcription(
            db,
            recording_id,
...
@app.post("/hotkey-recordings/{recording_id}/mark-processed", response_model=models.HotkeyRecordingResponse)
async def mark_hotkey_recording_processed(
    recording_id: str,
    db: Session = Depends(get_db),
):
    """Mark a hotkey recording as processed (converted to profile)."""
    recording = hotkey_recordings.mark_as_processed(db, recording_id)
    if not recording:
        raise HTTPException(status_code=404, detail="Recording not found")
    return recording


@app.delete("/hotkey-recordings/{recording_id}")
async def delete_hotkey_recording(
    recording_id: str,
    db: Session = Depends(get_db),
):
    """Delete a hotkey recording."""
    success = hotkey_recordings.delete_recording(db, recording_id)
    if not success:
        raise HTTPException(status_code=404, detail="Recording not found")
...
@app.post("/training/start", response_model=models.TrainingJobResponse)
async def start_training(
    data: models.TrainingJobCreate,
    db: Session = Depends(get_db),
):
    """Start a LoRA fine-tuning job for a voice profile."""
    # Check platform support
    supported, error_msg = training.can_train()
    if not supported:
        raise HTTPException(status_code=400, detail=error_msg)

    # Validate prerequisites
    valid, error_msg = training.validate_training_prerequisites(data.profile_id, db)
    if not valid:
        raise HTTPException(status_code=400, detail=error_msg)
    ...
    success, error_msg = training.start_training_job(
        job_id=job.id,
        profile_id=data.profile_id,
        num_epochs=data.num_epochs,
        learning_rate=data.learning_rate,
...
@app.get("/training/{job_id}", response_model=models.TrainingJobResponse)
async def get_training_job(
    job_id: str,
    db: Session = Depends(get_db),
):
    """Get training job status."""
    job = db.query(database.TrainingJob).filter_by(id=job_id).first()
    if not job:
...
async def cancel_training(job_id: str):
    ...
    success = training.cancel_training_job(job_id)
    if not success:
        raise HTTPException(status_code=404, detail="Training job not found or not running")
    return {"message": "Training job cancellation requested"}


@app.get("/training/progress/{job_id}")
async def get_training_progress(job_id: str):
    """Get training progress via Server-Sent Events."""
    progress_manager = get_progress_manager()

    async def event_generator():
        """Generate SSE events for training progress updates."""
...
@app.get("/profiles/{profile_id}/training-jobs", response_model=models.TrainingJobListResponse)
async def list_profile_training_jobs(
    profile_id: str,
    limit: int = 20,
    offset: int = 0,
    db: Session = Depends(get_db),
    ...
    query = db.query(database.TrainingJob).filter_by(profile_id=profile_id)
    total = query.count()
    items = query.order_by(database.TrainingJob.created_at.desc()).offset(offset).limit(limit).all()
    return models.TrainingJobListResponse(items=items, total=total)


@app.get("/profiles/{profile_id}/adapter", response_model=models.AdapterInfoResponse)
async def get_adapter_info(
    ...
    has_adapter = bool(profile.adapter_path and Path(profile.adapter_path).exists())

    # Find the most recent completed training job
    training_job = db.query(database.TrainingJob).filter_by(
        profile_id=profile_id,
        status="completed",
    ).order_by(database.TrainingJob.completed_at.desc()).first()

    return models.AdapterInfoResponse(
        profile_id=profile_id,
        has_adapter=has_adapter,
        adapter_path=profile.adapter_path if has_adapter else None,
        training_job_id=training_job.id if training_job else None,
        trained_at=training_job.completed_at if training_job else None,
    )


@app.delete("/profiles/{profile_id}/adapter")
async def delete_adapter(
    profile_id: str,
    db: Session = Depends(get_db),
):
    """Delete a trained adapter for a profile."""
    success = training.delete_adapter(profile_id, db)
    if not success:
        raise HTTPException(status_code=404, detail="Profile not found")
...
async def get_active_tasks():
    ...
    active_trainings = []
    for train_task in task_manager.get_active_trainings():
        active_trainings.append(models.ActiveTrainingTask(
            job_id=train_task.job_id,
            profile_id=train_task.profile_id,
            status=train_task.status,
            started_at=train_task.started_at,
        ))
...
@app.post("/tasks/clear-errors", response_model=models.ClearTasksResponse)
async def clear_error_tasks():
    """Clear all download tasks with error status."""
    task_manager = get_task_manager()
    cleared_count = task_manager.clear_error_downloads()
    logger.info(f"Cleared {cleared_count} error download tasks")
    return models.ClearTasksResponse(
        cleared_count=cleared_count,
        message=f"Cleared {cleared_count} error task(s)",
    )


@app.post("/tasks/clear-all", response_model=models.ClearTasksResponse)
async def clear_all_tasks():
    """Clear all download tasks (use with caution - will clear active downloads too)."""
    task_manager = get_task_manager()
    cleared_count = task_manager.clear_all_downloads()
    logger.info(f"Cleared {cleared_count} download tasks (including active ones)")
    return models.ClearTasksResponse(
        cleared_count=cleared_count,
        message=f"Cleared {cleared_count} task(s)",
    )
